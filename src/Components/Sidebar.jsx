...
import SubmenuReports from './SubmenuReports';
...
function Sidebar() {
  ...
  const [isManageClicked, setIsManageClicked] = useState(false); // Separate state for Manage Leaves
  const [isReportsClicked, setIsReportsClicked] = useState(false); // Separate state for Reports
  ...
  const toggleReportsDropdown = () => {
    setIsReportsClicked(!isReportsClicked);
  };
  ...
  return (
    <>
      {/* Sidebar */}
      <aside className="w-25% max-w-337.25px h-full bg-blue-500 text-white p-8">
        <h1 className="text-4xl font-bold mb-10 text-center">CKWD Leave Management System</h1>
        ...
        <nav>
          <ul>
            ...
            <li className="mb-5">
              <button onClick={toggleManageDropdown} className="flex items-center space-x-2 hover:bg-blue-700 p-2 rounded-md w-full justify-between">
                <div className="w-full flex flex-col gap-2">
                  <div className="w-full flex items-center justify-between">
                    ...
                    <span className={`transform ${isManageClicked ? 'rotate-180' : 'rotate-0'} transition-transform`}>▼</span>
                  </div>
                </div>
              </button>
              {isManageClicked && <Submenu />}
            </li>
            <li className="mb-5">
              <button onClick={toggleReportsDropdown} className="flex items-center space-x-2 hover:bg-blue-700 p-2 rounded-md w-full justify-between">
                <div className="w-full flex flex-col gap-2">
                  <div className="w-full flex items-center justify-between">
                    <div className="flex gap-2 items-center">
                    ...
                    <span className={`transform ${isReportsClicked ? 'rotate-180' : 'rotate-0'} transition-transform`}>▼</span>
                  </div>
                </div>
              </button>
              {isReportsClicked && <SubmenuReports />}
            </li>
            <li className="mb-5">
